...
$.ready(function() {
  ...
  $("#button_custom").addEventListener("click", function() {
    var w = Math.max(Math.min(parseInt(wElem.value), 500), 9);
    var h = Math.max(Math.min(parseInt(hElem.value), 500), 9);
    var m = Math.max(Math.min(parseInt(mElem.value), Math.floor(w * h * 0.6)), 1);
    wElem.value = String(w);
    hElem.value = String(h);
    mElem.value = String(m);
    minesweeper.new_game(w, h, m);
  });
